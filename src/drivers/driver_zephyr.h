...
struct zep_wpa_supp_mbox_msg_data {
	void *ctx;
	void *data;
	void (*cb)(void *ctx, void *data, int flags);
};
...
struct zep_wpa_supp_offload_ops {
	int (*disp_scan)(const struct device *dev, scan_result_cb_t cb);
};

struct zep_wpa_supp_dev_ops {
	struct ethernet_api if_api;
	struct zep_wpa_supp_offload_ops off_api;
	void *(*init)(void *supp_drv_if_ctx, const char *iface_name,
	...
	int (*deauthenticate)(void *if_priv, const char *addr,
			      unsigned short reason_code);
	int (*authenticate)(void *if_priv,
			    struct wpa_driver_auth_params *params);
	int (*associate)(void *if_priv,
			 struct wpa_driver_associate_params *params);
	int (*set_key)(void *if_priv, const unsigned char *ifname,
		       enum wpa_alg alg, const unsigned char *addr, int key_idx,
